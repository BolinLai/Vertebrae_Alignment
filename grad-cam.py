...
from models import Vgg16,  Customed_ShallowNet, CAM_CNN, ResNet18
...
#     ])
#
#     image = Image.fromarray(np.asarray(img)[:, :, 0])  # 得到的RGB图片三通道数值相等，只选择其中一个
#     image = trans(image)
#     image.unsqueeze_(0)  # 输入要求是4D的数据
#     input = Variable(image, requires_grad=True)
#
#     return input
def preprocess_image(img):
...
if __name__ == '__main__':
    ...
    if args.image_path == 'csv':  # 可以直接输入包含多张图片的csv文件
        ...
        cm = [[0, 0], [0, 0]]
        # for image, label, edge_mask, image_path in tqdm(test_dataloader):
        for image, label, image_path in tqdm(test_dataloader):
            ...
            # prob = softmax(model(image)[0], dim=1).detach().cpu().numpy()[0]
            prob = softmax(model(image), dim=1).detach().cpu().numpy()[0]

            if prob[int(label)] > 0.5:
                cm[int(label)][int(label)] += 1
            else:
                cm[int(label)][1 - int(label)] += 1

            # If None, returns the map for the highest scoring category.
            # Otherwise, targets the requested index.
            # target_index = None
            target_index = 0
            # target_index = 1 - np.argmax(prob)
            ...
            cam_save_path = '/'.join([*img_split_path[:8], 'CAM', 'CAM_0_' + args.model_path.split('/')[-1][:-4] + '-' + img_split_path[8], *img_split_path[9:11]])
            # cam_save_path = '/'.join([*img_split_path[:8], 'CAM_' + args.model_path.split('/')[-1][:-4] + '-' + img_split_path[8] + '-reverse', *img_split_path[9:11]])
            if not os.path.exists(cam_save_path):
                os.makedirs(cam_save_path)
            cam_save_name = img_split_path[-1][:-4] + '_' + str(round(prob[int(label)], 4)) + '.png'
            show_cam_on_image(img, mask, os.path.join(cam_save_path, cam_save_name))

        print(cm)
        print(cm[0][0] / sum(cm[0]), cm[1][1] / sum(cm[1]))
